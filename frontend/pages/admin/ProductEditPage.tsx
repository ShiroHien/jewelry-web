  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      try {
        ...
        setFormData(prev => ({ ...prev, images: [...prev.images, res.imageUrl] }));
      } catch (error) {
    ...
    <div className="container mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg shadow-md space-y-6">
            ...
            <div className="mt-2 grid grid-cols-4 gap-4">
                {formData.images.map((img, index) => (
                    <div key={index} className="relative group">
                        <img src={img} alt="" className="w-full h-32 object-cover rounded-md"/>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="availability" value={formData.availability} onChange={handleChange} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" required>
                    {Object.values(ProductAvailability).map(status => <option key={status} value={status}>{status}</option>)}
                </select>
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select name="category" value={formData.category} onChange={handleChange} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2">
                    {Object.values(ProductCategory).filter(c => c !== 'Blog').map(cat => <option key={cat} value={cat}>{cat}</option>)}
                </select>
            </div>
            <div>
        ...
        <div>
            ...
            <button type="button" onClick={addDetail} className="mt-4 text-sm text-indigo-600 hover:text-indigo-800">+ Add Detail</button>
        </div>
        
        {/* Actions */}
        <div className="flex justify-end">
            <button type="button" onClick={() => navigate('/admin/products')} className="bg-gray-200 text-gray-700 px-4 py-2 rounded-md mr-4">Cancel</button>
            <button type="submit" className="bg-gray-800 text-white px-4 py-2 rounded-md hover:bg-gray-700">Save Product</button>
        </div>
      </form>
    </div>
  );
